...
import { Download, Rocket, ShieldCheck, Zap, ArrowRight, BarChart3, Clock, Table as TableIcon, Search, Sun, Moon } from "lucide-react";
...
export default function NexusDashboard() {
  ...
  // Advanced Filter: Company + Search Term
  const filteredPlans = plans.filter((p) => {
    const matchesCompany = activeCompany === "All" || p.companies?.name === activeCompany;
  ...
  return (
    <div className="flex-1 w-full flex flex-col items-center bg-[#FDFDFD] dark:bg-black min-h-screen text-slate-900 dark:text-white pb-20 selection:bg-blue-500/30 transition-colors duration-300">
      
      {/* NAVIGATION */}
      <nav className="w-full flex justify-center border-b border-slate-200 dark:border-white/10 h-16 sticky top-0 bg-white/80 dark:bg-black/80 backdrop-blur-md z-40 px-6">
        <div className="w-full max-w-7xl flex justify-between items-center">
          <Link href="/" className="font-black text-xl tracking-tighter text-blue-600 dark:text-blue-500 uppercase">Nexus Intel</Link>
          <div className="flex items-center gap-6">
            ...
            <Link href="/reports" className="text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-white uppercase tracking-widest transition-colors">Oracle Archive</Link>
      ...
      <div className="w-full max-w-7xl px-6 flex flex-col gap-10 mt-12">
        <header className="flex flex-col gap-2">
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter leading-none text-slate-900 dark:text-white">
            Infrastructure <span className="text-blue-600 dark:text-blue-500">Oracle</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {/* CHART */}
            <section className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-8 shadow-xl dark:shadow-2xl">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                ...
                {companies.map((company) => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-xl text-[11px] font-bold transition-all border ${
                      activeCompany === company ? "bg-blue-600 border-blue-400 text-white" : "bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/5 text-slate-500 hover:text-blue-600"
                    }`}
                  >
                    {company}
                  </button>
              ...
              <div className="relative w-full md:w-64">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-[11px] outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white"
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>

              <button onClick={() => downloadIntelligenceReport(filteredPlans, activeCompany)} className="ml-auto flex items-center gap-2 px-5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[11px] font-bold hover:bg-emerald-600 hover:text-white transition-all">
                <Download size={14} /> Export Intel
              </button>
            </div>

            {/* COMPARISON MATRIX */}
            <section className="overflow-hidden rounded-[2.5rem] border border-slate-200 dark:border-white/5 bg-white dark:bg-slate-900/20">
              <div className="p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <TableIcon size={14} className="text-blue-600 dark:text-blue-500" />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {filteredPlans.slice(0, 10).map((plan, i) => (
                      <tr key={i} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td className="p-5 text-xs font-bold text-slate-900 dark:text-white">{plan.companies?.name}</td>
                        <td className="p-5 text-xs text-slate-500 dark:text-slate-400">{plan.plan_name}</td>
                        <td className="p-5 text-xs font-mono text-emerald-600 dark:text-emerald-400 font-bold">${plan.price_value}</td>
                        <td className="p-5 text-[9px] font-black text-slate-400 uppercase">Standardized</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <main className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredPlans.map((plan, i) => (
                <div key={i} className="p-8 rounded-[2.5rem] bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 hover:border-blue-500/50 transition-all flex flex-col group shadow-sm hover:shadow-xl">
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{plan.companies?.name}</span>
                    <Zap size={14} className="text-slate-300 dark:text-slate-800 group-hover:text-blue-500 transition-colors" />
                  </div>
                  <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6 leading-tight">{plan.plan_name}</h2>
                  <div className="flex items-baseline gap-1 mb-10">
                    <span className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">${plan.price_value}</span>
                    <span className="text-slate-500 text-[10px] uppercase font-bold tracking-widest">/ 1M Tokens</span>
                  </div>
                  <Link href="/reports" className="mt-auto pt-6 border-t border-slate-100 dark:border-white/5 text-[10px] font-black text-slate-400 hover:text-blue-600 transition-all flex items-center justify-between group/link uppercase tracking-[0.2em]">
          ...
          <aside className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="bg-white dark:bg-slate-900/30 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm">
              <div className="flex items-center gap-2 mb-6 text-slate-400">
                <Clock size={14} />
